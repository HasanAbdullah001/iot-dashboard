...
import { PowerIcon, TimerIcon, FanIcon, PlusIcon, WifiIcon } from './components/icons';
...
const TOPIC_CONTROL_TIMER = 'iot-home/control/timer/add';
...
// --- UI COMPONENTS ---
// NOTE: These are defined outside the main App component to prevent re-creation on every render.
...
const ConnectionManager: React.FC<ConnectionManagerProps> = ({ connect, disconnect, connectionStatus }) => {
    ...
    return (
        <div className="bg-gray-800 rounded-lg shadow-lg p-4 mb-6 transition-all duration-300">
            <button onClick={() => setIsExpanded(!isExpanded)} className="w-full text-left font-bold text-lg flex justify-between items-center">
            ...
            {isExpanded && (
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                    <div>
                        ...
                        <input type="text" name="url" value={config.url} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., ws://broker.hivemq.com" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-400">Port</label>
                        <input type="text" name="port" value={config.port} onChange={handleChange} className="mt-1 block w-full bg-gray-700 border border-gray-600 rounded-md shadow-sm py-2 px-3 text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 8000" required />
                    </div>
                     <div>
...
const App: React.FC = () => {
    ...
    const handleConnect = useCallback((config: BrokerConfig) => {
        ...
        try {
            ...
            // Fix: Replace 'Buffer' with an inline type definition to avoid depending on Node.js types in a browser environment.
            ...
            client.on('close', () => {
                if (connectionStatus !== 'Disconnected') {
                     // Avoid setting to disconnected if user intentionally disconnected
                }
            });
    ...
    // Timer color interpolation logic (Red -> Blue)
    const timerProgress = Math.min(iotStatus.timer / (60 * 60), 1); // Normalize timer to 0-1, max 60 mins
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <ControlCard title="Motor 1" icon={<FanIcon className="w-8 h-8 text-blue-400" />}>
                        <p className={`text-5xl font-bold ${iotStatus.motor1 === 'ON' ? 'text-green-400' : 'text-red-500'}`}>
                            {iotStatus.motor1}
                        </p>
                        <button
                            onClick={() => publishCommand(TOPIC_CONTROL_MOTOR1, iotStatus.motor1 === 'ON' ? 'OFF' : 'ON')}
                            ...
                            className={`w-32 h-32 rounded-full flex items-center justify-center transition-all duration-300 transform active:scale-95
                                ${iotStatus.motor1 === 'ON' ? 'bg-green-500 hover:bg-green-600 shadow-lg shadow-green-500/30' : 'bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30'}
                                ${!isConnected && 'bg-gray-600 cursor-not-allowed'}`}
                        >
                            <PowerIcon className="w-16 h-16 text-white" />
                        </button>
                    </ControlCard>

                    {/* Motor 2 Card */}
                     <ControlCard title="Motor 2" icon={<FanIcon className="w-8 h-8 text-blue-400" />}>
                        <p className={`text-5xl font-bold ${iotStatus.motor2 === 'ON' ? 'text-green-400' : 'text-red-500'}`}>
                            {iotStatus.motor2}
                        </p>
                        <button
                            onClick={() => publishCommand(TOPIC_CONTROL_MOTOR2, iotStatus.motor2 === 'ON' ? 'OFF' : 'ON')}
                            disabled={!isConnected}
                            className={`w-32 h-32 rounded-full flex items-center justify-center transition-all duration-300 transform active:scale-95
                                ${iotStatus.motor2 === 'ON' ? 'bg-green-500 hover:bg-green-600 shadow-lg shadow-green-500/30' : 'bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/30'}
                                ${!isConnected && 'bg-gray-600 cursor-not-allowed'}`}
                        >
                            <PowerIcon className="w-16 h-16 text-white" />
                        </button>
                    ...
                    <ControlCard title="Master Timer" icon={<TimerIcon className="w-8 h-8 text-blue-400" />}>
                       <div 
                         className="w-full text-center p-4 rounded-lg transition-colors duration-500"
                         style={{ backgroundColor: iotStatus.timer > 0 ? timerColor : 'transparent' }}
                       >
                         <p className="text-6xl font-mono tracking-widest text-white" >
                            {formatTime(iotStatus.timer)}
                         </p>
                       </div>
                        <button
                            onClick={() => publishCommand(TOPIC_CONTROL_TIMER, '5')}
                            disabled={!isConnected}
                            className="flex items-center justify-center w-full py-3 px-6 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md transition-transform transform active:scale-95 disabled:bg-gray-600 disabled:cursor-not-allowed"
                        >
